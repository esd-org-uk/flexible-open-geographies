...
{
    internal class ValidateTypeCode : IQuerySingle<string>
    {
        private readonly IContextFactory _contextFactory;
        private readonly string _code;

        public ValidateTypeCode(IContextFactory contextFactory, string code)
        {
        ...
        public string Find()
        {
            ...
            using (var context = _contextFactory.Create())
                return context.AreaTypes.AsNoTracking().Any(x => x.Code == _code)
                    ? "Code already exists"
                    : null;
        }
    }
